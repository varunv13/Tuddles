...
const Signup = () => {
  ...
  const handleSignup = async () => {
    try {
      ...
      const response = await axios.post(`${backend_url}/api/v1/users/auth/sign-up`, userData);
  ...
  return (
    ...
    <button
      onClick={toggleTheme}
      className={`p-2 rounded-full transition-all duration-300 ${theme === 'light' ? 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200' : 'bg-slate-700 text-yellow-300 hover:bg-slate-600'}`}
    >
      {theme === 'light' ? (
  ...
  </div><div className={`w-full max-w-md p-8 rounded-2xl shadow-xl ${theme === 'light' ? 'bg-white' : 'bg-gray-800'} backdrop-blur-lg transition-all duration-300`}>
      <div className="text-center mb-8">
        <h1 className={`text-3xl font-bold ${theme === 'light' ? 'text-indigo-600' : 'text-indigo-400'}`}>Create Account</h1>
        <p className={`mt-2 ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`}>Join our community today</p>
      </div>
      ...
      <div className="space-y-4 mb-6">
        <div className={`relative ${theme === 'light' ? 'text-gray-700' : 'text-gray-200'}`}>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-3 rounded-lg ${theme === 'light' ? 'bg-gray-50 border border-gray-200' : 'bg-gray-700 border border-gray-600'} focus:outline-none focus:ring-2 ${theme === 'light' ? 'focus:ring-indigo-500' : 'focus:ring-indigo-400'} transition-all`} />
          </div>
        </div>
        ...
        <div className={`relative ${theme === 'light' ? 'text-gray-700' : 'text-gray-200'}`}>
          <label htmlFor="confirmPassword" className="text-sm font-medium block mb-1">Confirm Password</label>
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 opacity-60" viewBox="0 0 20 20" fill="currentColor">
      ...
      <button
        ...
        disabled={isLoading}
        className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-300 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''} 
            ${theme === 'light'
      ...
      <div className="my-6 relative">
        ...
        <div className="relative flex justify-center text-sm">
          <span className={`px-2 ${theme === 'light' ? 'bg-white text-gray-500' : 'bg-gray-800 text-gray-400'}`}>Or continue with</span>
        </div>
      </div>
