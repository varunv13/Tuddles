...
// const Payment = () => {
//   const { pID, petID } = useParams();  // Capture both product and pet IDs from params
//   const [item, setItem] = useState(null);  // This will hold either a pet or product
//   const { state } = useLocation();
//   const [loading, setLoading] = useState(true);
//   const [discount, setDiscount] = useState(20);
//   const [error, setError] = useState(null);
...
//   const total_price = item?.price - (item?.price * discount) / 100;

//   useEffect(() => {
//     const fetchItem = async () => {
//       try {
//         const url = petID 
//           ? `${backend_url}/api/v1/pets/p/${petID}` 
//           : `${backend_url}/api/v1/products/p/${pID}`;
    
//         const response = await fetch(url);
//         if (!response.ok) throw new Error("Item not found");
//         const data = await response.json();
        
//         // Extract pet or product properly
//         setItem(data.data.pet || data.data.product);
//       } catch (err) {
//         setError(err.message);
//       } finally {
//         setLoading(false);
...
//       itemId: item._id,  // Use item._id for both products and pets
//       itemType: petID ? "Pet" : "Product",  // Determine if it's a pet or product order
//       quantity: 1,
//       shipping_Address: {
//         building_No: state.house,
//         street: state.street,
...
//                 <span>Order Total</span>
//                 <span>
//                   ₹{selectedPayment === "online" ? total_price : item?.price}
//                 </span>
//               </div>
//             </div>
//           </div>
...
//           {selectedPayment === "online" && (
//             <p className="text-green-700 font-medium mb-6 text-center">
//               🎉 You saved ₹{(item?.price * discount) / 100} on this order!
//             </p>
//           )}
